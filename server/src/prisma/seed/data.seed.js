...
const generateDayRecords = (dateStr, startHourWib = 0) => {
  ...
  for (let slot = START_SLOT; slot < TOTAL_SLOTS; slot++) {
    ...
    // ── Penyiraman Timer jam 07:00:00–07:00:14 WIB ────────
    const timerActive = isWateringSlot(hourWib, min, sec)
    ...
    records.push({
      ...
      humidity,
      soil,
      pump: timerActive ? 'VERYHIGH' : pump, // override pump saat Timer
      fan, // fan tidak berubah saat Timer
      humidifier, // humidifier tidak berubah saat Timer
      pumpMode: timerActive ? 'Timer' : 'Fuzzy', // mode khusus pump
      fanMode: 'Fuzzy',
      humidifierMode: 'Fuzzy',
    })
  }
...
const seed = async () => {
  ...
  const delAct = await prisma.actuatorLog.deleteMany({
    where: { date: { gte: START_DATE, lte: END_DATE } },
  })
  const delData = await prisma.data.deleteMany({
    where: { date: { gte: START_DATE, lte: END_DATE } },
  })
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    const records = generateDayRecords(dateStr, startHourWib).filter(
      (r) => r.date >= START_DATE && r.date <= END_DATE,
    )
    ...
    for (let j = 0; j < records.length; j += BATCH_SIZE) {
      ...
      const inserted = await prisma.data.findMany({
        ...
        select: { id: true, date: true, pump: true, fan: true, humidifier: true },
        orderBy: { date: 'asc' },
      })

      // 3. Map inserted by date ISO string untuk ambil mode yang benar
      const batchByDate = new Map(batch.map((r) => [r.date.toISOString(), r]))

      // 4. Insert ActuatorLog batch dengan mode yang sesuai
      const logPayload = inserted.flatMap((d) => {
        const rec = batchByDate.get(d.date?.toISOString())
        const pumpMode = rec?.pumpMode ?? 'Fuzzy'
        const fanMode = rec?.fanMode ?? 'Fuzzy'
        const humidifierMode = rec?.humidifierMode ?? 'Fuzzy'

        if (pumpMode === 'Timer') timerCount++

        return [
          { type: 'pump', status: d.pump, mode: pumpMode, dataId: d.id },
          { type: 'fan', status: d.fan, mode: fanMode, dataId: d.id },
          { type: 'humidifier', status: d.humidifier, mode: humidifierMode, dataId: d.id },
        ]
      })
